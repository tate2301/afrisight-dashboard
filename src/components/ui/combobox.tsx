...
interface ComboboxProps<T> {
    ...
    renderItem?: (item: ComboboxItem<T>, isSelected: boolean, handleSelect: (value: string) => void) => React.ReactNode;
            ...
            <PopoverContent className="w-96 p-0">
                <Command>
                    ...
                    <CommandList className="p-1">
                        ...
                        <CommandGroup>
                            {items.map((item) => (
                                renderItem ? renderItem(item, value === item.value, handleSelect) : (
                                    <CommandItem
                                        key={item.value}
                                        value={item.value}
