...
export function Label(props: LabelProps) {
	return (
		<RACLabel
			{...props}
			className={twMerge(
				'text-gray-500 dark:text-zinc-400 font-medium cursor-default w-fit',
				props.className,
			)}
		/>
	);
	...
	return (
		<RACFieldError
			...
			className={composeTailwindRenderProps(
				props.className,
				'text-red-600 forced-colors:text-[Mark]',
			)}
		/>
	);
}
...
export function Input(props: InputProps) {
	return (
		<RACInput
			...
			className={composeTailwindRenderProps(
				props.className,
				'px-2 py-1.5 flex-1 min-w-0 outline outline-0 bg-white dark:bg-zinc-900 text-gray-800 dark:text-zinc-200 disabled:text-gray-200 dark:disabled:text-zinc-600',
			)}
		/>
	);
}
