...
function Filter({ clause, field, operator, value, onDelete, columns, onUpdate }: FilterProps) {
    ...
    return (
        <div className="flex items-center space-x-2 mb-2">
            ...
            >
                ...
                <SelectContent>
                    {columns.map((col) => (
                        <SelectItem key={col.label} value={col.label}>{col.label}</SelectItem>
                    ))}
                </SelectContent>
            </Select>
            <Select
            ...
            >
                ...
                <SelectContent>
                    {selectedColumn && operators[selectedColumn.type].map((op) => (
            ...
            <Button variant="ghost" color="red" radius="full" size="1" onClick={onDelete}>
...
export function FilterBuilder({ columns }: FilterBuilderProps) {
    const [filters, setFilters] = useState<Omit<FilterProps, "columns" | "onUpdate" | "onDelete">[]>([
    ])
    const [open, setOpen] = useState(false)

    const addFilter = () => {
        setFilters([...filters, { clause: "AND", field: columns[0].label, operator: operators[columns[0].type][0], value: "" }])
    }
