...
const Sidebar = () => {
    return (
        <Flex direction={"column"} className="bg-zinc-50 sticky top-0 h-screen" css={{
            borderRight: "1px solid $gray2",
            width: SIDEBAR_WIDTH,
            gap: 0
        }}>
            <Flex className="relative items-center" css={{ height: NAVBAR_HEIGHT }}>
                <Image src={"/cx_mappers_logo.svg"} width={44} height={44} alt="CX Mappers Logo" className="mr-2" />
                <Paragraph weight={"bold"} color={"primary"} className="tracking-tight">
                    CX Mappers
                </Paragraph>
            </Flex>
            <Separator />
            ...
            <Flex direction={"column"} className="p-2 space-y-2">
                <Button colorScheme={"surface"} variant={"outline"} css={{ backgroundColor: "$white" }}>
                    <PlusSmallIcon className="size-4 mr-2" />
                    Add a member
                </Button>
            </Flex>
        </Flex>
...
const SidebarNavItem = ({ Icon, text, href }: SidebarNavItemProps) => {
    ...
    return (
        <Link href={href}>
            <ListItem className={cn("rounded-md hover:bg-zinc-400/20", active && "bg-zinc-400/20 hover:bg-zinc-100")}>
