		...
		<form
			...
			<div className="flex flex-col gap-2 mb-4">
				...
				<p className="text-content-secondary">
					Set up how rewards are given to participants.
				</p>
			</div>
			<Separator className="mb-8" />
			<div className="mb-8">
				<h3 className="font-bold mb-4">Active Reward Policy</h3>
				<div className="p-4 rounded-xl bg-surface-secondary select-shadow">
					<p className="text-base font-medium mb-1">{gig.rewardPolicy?.name}</p>
					...
					<div className="flex gap-8">
						<p className="text-base font-bold px-2 py-1 rounded-lg bg-surface-quaternary items-center inline-flex">
							<Symbol className="!text-base mr-2">payments</Symbol>{' '}
							{Intl.NumberFormat('en-us', {
								currency: 'USD',
								currencySign: 'accounting',
								style: 'currency',
							}).format(gig.rewardPolicy?.dollarValue ?? 0)}
						</p>
						<p className="text-base font-bold px-2 py-1 rounded-lg bg-surface-quaternary items-center inline-flex">
							<Symbol className="!text-base mr-2">kid_star</Symbol>{' '}
							{gig.rewardPolicy?.pointsValue} points
						</p>
					</div>
				</div>
			</div>
